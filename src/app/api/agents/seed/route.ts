...
const MASTER_AGENT_CONFIG = {
  ...
  description: "AI-powered project consultant that guides clients through requirements gathering and project scoping. Creates leads, project requests, and initiates the project lifecycle.",
  ...
  try {
    ...
    if (existing) {
      return NextResponse.json({
        ...
        agent: { id: existing._id, name: existing.name, slug: existing.slug },
      });
    }
    ...
    if (!adminUser) {
      return NextResponse.json({ error: "No admin user found. Run seed script first." }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      agent: { id: agent._id, name: agent.name, slug: agent.slug },
    });
  } catch (error) {
    console.error("Seed Master Agent error:", error);
    return NextResponse.json({ error: "Failed to seed Master Agent" }, { status: 500 });
  }
}
