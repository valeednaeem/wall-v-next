...
interface ClientProject {
  ...
  budget?: number;
  currency?: string;
  stages: { name: string; status: string; order: number }[] | string[];
  currentStage?: { name: string } | string;
  financial?: {
    quotedAmount: number;
    paidAmount: number;
  ...
  requirements: { title: string; status: string; scope: string }[] | string[];
  changeRequests: { title: string; status: string; createdAt: string }[] | string[];
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {selectedProject.scope.features.map((f, i) => (
                          <span key={i} className="px-2 py-0.5 text-[10px] bg-violet-50 text-violet-700 rounded-full">{f}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div className="space-y-2">
                      {selectedProject.stages.map((stage, i) => {
                        ...
                        const order = typeof stage === "string" ? i + 1 : stage.order;
                        return (
                          <div key={i} className="flex items-center gap-3 p-2 rounded-lg">
                            <span className="text-xs text-gray-400 w-6">{order}</span>
                  ...
                  <div className="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div className="space-y-1">
                      {selectedProject.requirements.map((req, i) => {
                        ...
                        const reqStatus = typeof req === "string" ? "" : req.status;
                        const scope = typeof req === "string" ? "in-scope" : req.scope;
                        return (
                          <div key={i} className="flex items-center gap-2 text-sm p-1">
                            <span className={`w-2 h-2 rounded-full ${scope === "in-scope" ? "bg-emerald-400" : "bg-red-400"}`} />
                            <span className="flex-1">{title}</span>
                            {reqStatus && <span className="text-xs text-gray-400 capitalize">{reqStatus}</span>}
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div className="space-y-2">
                      {selectedProject.changeRequests.map((cr, i) => {
                        const title = typeof cr === "string" ? cr : cr.title;
                        const crStatus = typeof cr === "string" ? "" : cr.status;
                        return (
                          <div key={i} className="flex items-center justify-between p-2 rounded-lg border border-gray-100">
                            <span className="text-sm">{title}</span>
                            {crStatus && (
                              <span className={`px-2 py-0.5 text-[10px] rounded-full ${
                                crStatus === "approved" ? "bg-emerald-100 text-emerald-700" :
                                crStatus === "rejected" ? "bg-red-100 text-red-700" :
