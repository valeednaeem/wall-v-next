...
export default function HostingOffersPage() {
  ...
  const createOffer = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setNewOffer({ planId: "", provider: "resellerspanel", name: "", offerPrice: 0, validUntil: "", description: "" });
        fetchOffers();
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Hosting Offers</h1>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-xl font-semibold mb-4">Create Hosting Offer</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Plan ID</label>
                <input
                  type="text"
                  value={newOffer.planId}
                  onChange={(e) => setNewOffer({ ...newOffer, planId: e.target.value })}
                  placeholder="e.g., rp-starter"
